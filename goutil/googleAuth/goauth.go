...
func New() (*GoogleAuth, error) {
	seed := _GenerateSeed()
	secretKey, e := _GenerateSecretKey(seed)
	return &GoogleAuth{
		Seed:      seed,
		SecretKey: secretKey,
	}, e
...
func (g *GoogleAuth) GetNewCode() uint32 {
	return generateCode(g.SecretKey, time.Now().Unix(), false)
}
...
func _SHA1PRNG(seed []byte, size int) ([]byte, error) {
	hashs := _SHA1(_SHA1(seed))
	maxLen := len(hashs)
	realLen := size
	if realLen > maxLen {
...
func _GenerateSecretKey(seed string) (string, error) {
	...
	key, err := _SHA1PRNG(decodeSeed, secretSize)
	if err != nil {
		return "", err
	}
 ...
 * @生成code验证码
 * @secretKey 		密钥
 * @curTimeSlice 	当前时间戳 - gotime.Unix()
 ...
 */
func _GetNewCode(secretKey string, curTimeSlice int64) uint32 {
 ...
 * @生成密钥种子
 */
func _GenerateSeed() string {
	mu.Lock()
	curTimeSlice := time.Now().Unix() + int64(counter)
	curTimeSliceBytes := toBytes(curTimeSlice)
	hash := _SHA512(curTimeSliceBytes)
	coder := base64.NewEncoding(base64Table)
